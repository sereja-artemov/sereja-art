...
import { DarkModeContextType, useTheme } from '@/context/darkModeContext';
import { navigationRoutes } from '@/data/navigationRoutes';

const Header: React.FC = () => {
  const { theme, currentTheme, setDarkTheme, setLightTheme }: DarkModeContextType = useTheme();
          ...
          <ul className={s.list}>
            {navigationRoutes.map((element, index) => (
              <li className={s.listItem} key={index}>
                <Link onClick={closeMobileMenu} href={element.route}
                      className={`${s.link} ${activeLink(element.route, router.pathname)}`}>
